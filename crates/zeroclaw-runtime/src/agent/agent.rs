...
impl Agent {
    ...
    pub fn seed_history(&mut self, messages: &[ChatMessage]) {
        if self.history.is_empty()
            && let Ok(item) = self.build_system_context_item()
        {
            self.history
    ...
    async fn build_turn_context(
        ...
        if let Some(hooks) = self.hook_runner.as_ref() {
            match hooks
    ...
    pub async fn turn(&mut self, user_message: &str) -> Result<String> {
        ...
        if self.history.is_empty()
            && let Some(system_prompt) = context_build.context_package.system_prompt.clone()
        {
            self.history
    ...
    pub async fn turn_streamed(
        ...
        let context_build = self
            .build_turn_context(user_message, format!("[{now}]"))
            .await?;
...
mod tests {
    ...
    use crate::hooks::{ContextHookResult, HookHandler, HookRunner};
