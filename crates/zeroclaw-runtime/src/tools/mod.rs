...
struct SandboxCommandWrapper {
    sandbox: Arc<dyn Sandbox>,
}

impl zeroclaw_tools::browser_delegate::CommandWrapper for SandboxCommandWrapper {
    fn wrap_command(&self, cmd: &mut std::process::Command) -> std::io::Result<()> {
        self.sandbox.wrap_command(cmd)
    }
}
...
pub fn all_tools_with_runtime(
    ...
    if root_config.browser_delegate.enabled {
        if has_shell_access {
            tool_arcs.push(Arc::new(BrowserDelegateTool::new_with_command_wrapper(
                security.clone(),
                root_config.browser_delegate.clone(),
                Arc::new(SandboxCommandWrapper {
