...
impl AcpServer {
    ...
    async fn handle_session_new(&self, params: &Value) -> RpcResult {
        ...
        let workspace_path = if let Some(project_root) = project_root {
            zeroclaw_config::project_root::apply_project_root(
                &mut session_config,
                project_root.as_ref(),
            )
            .map_err(|e| RpcError {
            ...
            std::fs::canonicalize(&requested_cwd).map_err(|e| RpcError {
                code: INVALID_PARAMS,
                message: format!(
...
mod tests {
    ...
    #[tokio::test]
    async fn session_new_accepts_project_root_alias() {
