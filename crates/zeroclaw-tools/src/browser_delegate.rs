...
pub struct NoopCommandWrapper;

impl CommandWrapper for NoopCommandWrapper {
...
pub struct BrowserDelegateTool {
    ...
    command_wrapper: Arc<dyn CommandWrapper>,
}
...
impl BrowserDelegateTool {
    ...
    pub fn new(security: Arc<SecurityPolicy>, config: BrowserDelegateConfig) -> Self {
        Self::new_with_command_wrapper(security, config, Arc::new(NoopCommandWrapper))
    }

    pub fn new_with_command_wrapper(
        security: Arc<SecurityPolicy>,
        config: BrowserDelegateConfig,
        command_wrapper: Arc<dyn CommandWrapper>,
    ) -> Self {
    ...
    fn build_command(
        ...
        self.command_wrapper
            ...
            .map_err(|error| anyhow::anyhow!("command wrapper error: {error}"))?;
...
mod tests {
    ...
    #[test]
    fn build_command_applies_command_wrapper_when_configured() {
        #[derive(Default)]
        struct PrefixWrapper;

        impl CommandWrapper for PrefixWrapper {
            fn wrap_command(&self, cmd: &mut std::process::Command) -> std::io::Result<()> {
        ...
        let tool = BrowserDelegateTool::new_with_command_wrapper(
            Arc::new(SecurityPolicy::default()),
            default_test_config(),
            Arc::new(PrefixWrapper),
        );
    ...
    #[test]
    fn build_command_returns_error_when_command_wrapper_fails() {
        struct FailingWrapper;

        impl CommandWrapper for FailingWrapper {
            fn wrap_command(&self, _cmd: &mut std::process::Command) -> std::io::Result<()> {
                Err(std::io::Error::other("wrapper boom"))
            }
        }
        ...
        let tool = BrowserDelegateTool::new_with_command_wrapper(
            Arc::new(SecurityPolicy::default()),
            default_test_config(),
            Arc::new(FailingWrapper),
        );

        let result = tool.build_command("read inbox", None);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("command wrapper"));
    }
