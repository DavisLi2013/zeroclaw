...
**Per-session sandbox roots (ACP and gateway WebSocket):** When a session is opened via ACP, `session/new` can bind either a full project root (`project_root` / `projectRoot`) or a compatibility cwd (`cwd`, `workspaceDir`, `workspace_dir`). In both cases, that path becomes the `SecurityPolicy` workspace boundary for all file and shell tools for the lifetime of the session. With ACP `project_root`, ZeroClaw also clones the base config and redirects workspace-scoped runtime state under `project_root/.zeroclaw/config.toml`. With ACP `cwd` and gateway WebSocket `cwd`, the daemon's global `workspace_dir` remains the data directory for memory, identity, cron, and other persistent state.

**Important:** the `project_root` or `cwd` parameter changes which directory on the **ZeroClaw host** the agent is sandboxed to - it does not affect which machine tools run on. Tool use (shell commands, file reads/writes) always executes on the machine running ZeroClaw. If you connect to a remote ZeroClaw instance over ACP WebSocket or the gateway WebSocket, tool calls operate on the remote machine's filesystem, not on your local machine. For localhost-only deployments this distinction does not matter, but remote setups should account for it.
